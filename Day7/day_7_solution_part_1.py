...
HIGH_TO_LOW_CARD_VALUE = ["A", "K", "Q", "J", "T", "9", "8", "7", "6", "5", "4", "3", "2"]


def custom_sort_key(item):
    return [HIGH_TO_LOW_CARD_VALUE.index(char) for char in item]
...
def is_four_of_a_kind(counter):
    return len(counter) == 2 and max(counter.values()) == 4


def is_full_house(counter):
    return len(counter) == 2 and not max(counter.values()) == 4


def is_three_of_a_kind(counter):
    return len(counter) == 3 and max(counter.values()) == 3


def is_two_pair(counter):
    return len(counter) == 3 and not max(counter.values()) == 3
